...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const fd = formData ? encodeURIComponent(formData) : '';

    const session = await stripe.checkout.sessions.create({
      payment_method_types: ['card'],
      mode: 'payment',
      line_items: [{ price_data: { currency: 'aud', product_data: { name: TIERS[tier].name }, unit_amount: TIERS[tier].price }, quantity: 1 }],
      success_url: `${base}/?paid=true&tier=${tier}&fd=${fd}`,
      cancel_url: `${base}/`,
